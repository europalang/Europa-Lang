    ...
    fn next_char(&mut self) -> Result<(), Error> {
        ...
        match char {
            ...
            '"' | '\'' => {
                ...
                let mut templates: HashMap<usize, Vec<Token>> = HashMap::new();
                ...
                while self.is_valid() && self.peek() != str_type {
                    ...
                    if self.peek() == '\\' {
                    ...
                    } else if self.peek() == '#' && self.peek_n(1) == '{' {
                        let i = self.i - start;
                        let lf = self.info;
                        self.next(); // #
                        self.next(); // {
                        ...
                        if !self.is_valid() {
                            return Err(Error::new_n(
                                self.info,
                                String::from("Unterminated template."),
                                ErrorType::SyntaxError,
                                vec![ErrorNote::Expect(lf, "Template starts here.".into())],
                            ));
                        }

                        let mut lexer = Lexer::new(&code);
                        lexer.info = self.info;
                        let slice = lexer.init()?;
                        let tokens = slice[0..slice.len() - 1].to_vec(); // ignore the eof

                        templates.insert(i, tokens);
                ...
                self.tokens.push(Token {
                    ttype: if is_template {
                        TType::TemplateString(string, templates)
                    } else {
                        TType::String(string)
                    },
                    lineinfo: self.info,
